...
function fullsocket(){
    ...
    socket.addEventListener('message', e=>{
        ...
        if(obj.action == "playerList"){
            ...
            // app.bolas_chamadas = obj.payload.drawnNumbers
            // app.bolas_chamadas_r = inverse(obj.payload.drawnNumbers)
            // card.card_check_all_amarelo(obj.payload.drawnNumbers)
            // app.isLoading = false

            // bt_play.removeAttribute('style')
            // bar_loading_out.style.display='none'
        }
        
        if(obj.action == 'number'){
            Play.number(obj.payload.number)
            // chama(obj.payload.number)
            // chamas(obj.payload.drawnNumbers)
            // audio.play(obj.payload.number)
            // app.bolas_chamadas = obj.payload.drawnNumbers
            // app.bolas_chamadas_r = inverse(obj.payload.drawnNumbers)
        }
